...
class GANom:
	...
	def toString(self):
		return self.article
	def getVar(self,var):
		if var in ['status','reviewpage','reviewer','subtopic','unixtime','subtopic','nominator','nominator_plain']:
			return var
		return False
	...
	def wikicode(self):
		code = "# {{GANentry|1=" + self.article + "|2=" + self.reviewpage + self.existsThingyGahhh() + "}} " + self.numOfReviews(self.nominator_plain) + self.nominator + " " + self.timestamp + "\n"
		...
		if currentNom.getVar('status') is 'new':
			...
			and not currentNom.getVar('reviewer') in dontNotify):
				...
				if allow_bots(noms_talk_page.content(),'GA bot'):
					page.save(noms_talk_page.content() + "\n\n" + msg, summary="/* Your [[WP:GA|GA]] nomination of [[" + currentNom + "]] */ new section")
